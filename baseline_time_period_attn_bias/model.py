...
class ModelInput(NamedTuple):
    ...
    seq_hour_buckets: Optional[dict] = None  # {domain: tensor [B, L]}
    seq_dow_buckets: Optional[dict] = None   # {domain: tensor [B, L]}
...
class RoPEMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.0,
        rope_on_q: bool = True,
        use_time_attn_bias: bool = False,
        num_time_buckets_for_bias: int = 0,
    ) -> None:
        super().__init__()
        ...
        self.use_time_attn_bias = use_time_attn_bias
        ...
        if self.use_time_attn_bias:
            self.time_attn_bias = nn.Embedding(
                num_time_buckets_for_bias, num_heads, padding_idx=0)
            nn.init.zeros_(self.time_attn_bias.weight.data)
    ...
    def forward(
        ...
        if self.use_time_attn_bias and time_bucket_ids is not None:
            time_bias = self.time_attn_bias(time_bucket_ids)  # (B, Lk, H)
            time_bias = time_bias.permute(0, 2, 1).unsqueeze(2)  # (B, H, 1, Lk)
            time_bias = time_bias.expand(B, self.num_heads, Lq, Lk)
            if sdpa_attn_mask is None:
                sdpa_attn_mask = time_bias
            elif sdpa_attn_mask.dtype == torch.bool:
                additive = torch.zeros(
                    B, self.num_heads, Lq, Lk, device=query.device, dtype=query.dtype)
                additive = additive.masked_fill(~sdpa_attn_mask, float('-inf'))
                sdpa_attn_mask = additive + time_bias
            else:
                sdpa_attn_mask = sdpa_attn_mask + time_bias
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = RoPEMultiheadAttention(
            ...
            num_time_buckets_for_bias=num_time_buckets_for_bias,
        )
...
class SwiGLUEncoder(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        x = self.dropout(x)
        x = residual + x
        return x, key_padding_mask, kwargs.get('time_bucket_ids', None)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        return x, key_padding_mask, time_bucket_ids
...
class LongerEncoder(nn.Module):
    ...
    def forward(
        ...
        if L > self.top_k:
            ...
            new_time_bucket_ids = None
            if time_bucket_ids is not None:
                new_time_bucket_ids = torch.gather(time_bucket_ids, dim=1, index=q_pos_indices)
                new_time_bucket_ids = new_time_bucket_ids.masked_fill(new_mask, 0)
        else:
            # === Self Attention mode (subsequent MultiSeqHyFormerBlocks) ===
            new_mask = key_padding_mask
            new_time_bucket_ids = time_bucket_ids
        ...
        return out, new_mask, new_time_bucket_ids
...
def create_sequence_encoder(
    ...
    if encoder_type == 'swiglu':
        ...
        return TransformerEncoder(
            d_model, num_heads, hidden_mult, dropout,
            use_time_attn_bias=use_time_attn_bias,
            num_time_buckets_for_bias=num_time_buckets_for_bias)
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def forward(
        ...
        seq_tokens_list: list,
        seq_padding_masks: list,
        seq_time_bucket_ids_list: Optional[list] = None,
        ...
        for i in range(S):
            ...
            decoded_q_i = self.cross_attns[i](
                ...
                rope_cos=rc, rope_sin=rs, time_bucket_ids=next_time_bucket_ids[i],
            )
        ...
        return next_q_list, next_ns, next_seqs, next_masks, next_time_bucket_ids
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.use_time_period_features:
            self.hour_embedding = nn.Embedding(25, d_model, padding_idx=0)
            self.dow_embedding = nn.Embedding(8, d_model, padding_idx=0)
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                use_time_attn_bias=use_time_attn_bias,
                num_time_buckets_for_bias=num_time_buckets,
            )
    ...
    def _init_params(self) -> None:
        ...
        if self.use_time_period_features:
            ...
            nn.init.xavier_normal_(self.dow_embedding.weight.data)
            self.dow_embedding.weight.data[0, :] = 0
    ...
    def _embed_seq_domain(
        ...
        dow_bucket_ids: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        if self.use_time_period_features and hour_bucket_ids is not None:
            token_emb = token_emb + self.hour_embedding(hour_bucket_ids)
        if self.use_time_period_features and dow_bucket_ids is not None:
            token_emb = token_emb + self.dow_embedding(dow_bucket_ids)
    ...
    def _run_multi_seq_blocks(
        ...
        curr_time_bucket_ids = seq_time_bucket_ids_list
        ...
        for block in self.blocks:
            ...
            curr_qs, curr_ns, curr_seqs, curr_masks, curr_time_bucket_ids = block(
                q_tokens_list=curr_qs,
                ns_tokens=curr_ns,
                seq_tokens_list=curr_seqs,
                seq_padding_masks=curr_masks,
                seq_time_bucket_ids_list=curr_time_bucket_ids,
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        seq_tokens_list = []
        seq_masks_list = []
        seq_time_bucket_ids_list = []
        for domain in self.seq_domains:
            seq_time_bucket_ids_list.append(inputs.seq_time_buckets[domain])
            hour_bucket_ids = None
            if inputs.seq_hour_buckets is not None:
                hour_bucket_ids = inputs.seq_hour_buckets.get(domain)
            dow_bucket_ids = None
            if inputs.seq_dow_buckets is not None:
                dow_bucket_ids = inputs.seq_dow_buckets.get(domain)
            tokens = self._embed_seq_domain(
                inputs.seq_data[domain],
                self._seq_embs[domain], self._seq_proj[domain],
                self._seq_is_id[domain], self._seq_emb_index[domain],
                inputs.seq_time_buckets[domain],
                hour_bucket_ids=hour_bucket_ids,
                dow_bucket_ids=dow_bucket_ids)
        ...
        output = self._run_multi_seq_blocks(
            ...
            seq_time_bucket_ids_list=seq_time_bucket_ids_list,
