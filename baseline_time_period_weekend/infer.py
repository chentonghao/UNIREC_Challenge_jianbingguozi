"""PCVRHyFormer inference script for the baseline_time_period_weekend variant.
...
try:
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_weekend.dataset import (
        FeatureSchema,
        NUM_TIME_BUCKETS,
        PCVRParquetDataset,
    )
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_weekend.model import (
        ModelInput,
        PCVRHyFormer,
    )
...
_FALLBACK_MODEL_CFG = {
    ...
    'use_hour_x_weekend_bucket': False,
...
def _batch_to_model_input(batch: Dict[str, Any], device: str) -> ModelInput:
    ...
    seq_hour_weekend_buckets: Optional[Dict[str, torch.Tensor]] = None
    ...
    if has_dow_buckets:
        seq_dow_buckets = {}
    has_hour_weekend_buckets = any(f'{domain}_hour_weekend_bucket' in device_batch for domain in seq_domains)
    if has_hour_weekend_buckets:
        seq_hour_weekend_buckets = {}
    for domain in seq_domains:
        ...
        if seq_hour_weekend_buckets is not None and f'{domain}_hour_weekend_bucket' in device_batch:
            seq_hour_weekend_buckets[domain] = device_batch[f'{domain}_hour_weekend_bucket']
    ...
    return ModelInput(
        ...
        seq_hour_weekend_buckets=seq_hour_weekend_buckets,
    )
...
def main() -> None:
    ...
    use_weekend_feature = bool(train_config.get('use_weekend_feature', False))
    use_hour_x_weekend_bucket = bool(train_config.get('use_hour_x_weekend_bucket', False))
