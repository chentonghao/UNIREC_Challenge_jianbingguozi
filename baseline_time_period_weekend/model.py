...
class ModelInput(NamedTuple):
    ...
    seq_hour_weekend_buckets: Optional[dict] = None  # {domain: tensor [B, L]}
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.use_time_period_features and self.use_hour_x_weekend_bucket:
            self.hour_weekend_embedding = nn.Embedding(49, d_model, padding_idx=0)
        elif self.use_time_period_features:
    ...
    def _init_params(self) -> None:
        ...
        if self.use_time_period_features and self.use_hour_x_weekend_bucket:
            nn.init.xavier_normal_(self.hour_weekend_embedding.weight.data)
            self.hour_weekend_embedding.weight.data[0, :] = 0
        elif self.use_time_period_features:
    ...
    def _embed_seq_domain(
        ...
        hour_weekend_bucket_ids: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        if (
            self.use_time_period_features
            and self.use_hour_x_weekend_bucket
            and hour_weekend_bucket_ids is not None
        ):
            token_emb = token_emb + self.hour_weekend_embedding(hour_weekend_bucket_ids)
        elif self.use_time_period_features and hour_bucket_ids is not None:
            token_emb = token_emb + self.hour_embedding(hour_bucket_ids)
        if (
            self.use_time_period_features
            and not self.use_hour_x_weekend_bucket
            and dow_bucket_ids is not None
        ):
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            ...
            hour_weekend_bucket_ids = None
            if inputs.seq_hour_weekend_buckets is not None:
                hour_weekend_bucket_ids = inputs.seq_hour_weekend_buckets.get(domain)
            tokens = self._embed_seq_domain(
                ...
                hour_weekend_bucket_ids=hour_weekend_bucket_ids)
