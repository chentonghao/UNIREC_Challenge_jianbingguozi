...
REQUEST_TIME_PERIOD_DENSE_DIM = 4
NUM_HOUR_BUCKETS = 25
NUM_DOW_BUCKETS = 8
NUM_HOUR_WEEKEND_BUCKETS = 49
SECONDS_PER_HOUR = 3600
SECONDS_PER_DAY = 86400
MONDAY_OFFSET_FROM_UNIX_EPOCH = 3
REQUEST_TIME_PERIOD_FID = -1
REQUEST_WEEKEND_FID = -2
VALID_TIME_PERIOD_MODES = {'off', 'request_only', 'seq_only', 'full'}


def _extract_hour_and_dow(
    timestamps: "npt.NDArray[np.int64]",
) -> Tuple["npt.NDArray[np.int64]", "npt.NDArray[np.int64]"]:
    """Extract hour-of-day and day-of-week (Monday=0) from Unix seconds."""
    flat_ts = timestamps.reshape(-1).astype(np.int64, copy=False)
    hours = np.zeros_like(flat_ts)
    dows = np.zeros_like(flat_ts)

    valid_mask = flat_ts > 0
    if valid_mask.any():
        valid_ts = flat_ts[valid_mask]
        hours[valid_mask] = (valid_ts // SECONDS_PER_HOUR) % 24
        dows[valid_mask] = ((valid_ts // SECONDS_PER_DAY) + MONDAY_OFFSET_FROM_UNIX_EPOCH) % 7

    return hours.reshape(timestamps.shape), dows.reshape(timestamps.shape)


def _build_request_time_period_dense(
    timestamps: "npt.NDArray[np.int64]",
) -> "npt.NDArray[np.float32]":
    """Build [hour_sin, hour_cos, dow_sin, dow_cos] from request timestamps."""
    feats = np.zeros((timestamps.shape[0], REQUEST_TIME_PERIOD_DENSE_DIM), dtype=np.float32)
    valid_mask = timestamps > 0
    if not valid_mask.any():
        return feats

    hours, dows = _extract_hour_and_dow(timestamps)
    hour_angles = 2.0 * np.pi * hours[valid_mask].astype(np.float32) / 24.0
    dow_angles = 2.0 * np.pi * dows[valid_mask].astype(np.float32) / 7.0
    feats[valid_mask, 0] = np.sin(hour_angles)
    feats[valid_mask, 1] = np.cos(hour_angles)
    feats[valid_mask, 2] = np.sin(dow_angles)
    feats[valid_mask, 3] = np.cos(dow_angles)
    return feats


def _build_period_bucket_ids(
    timestamps: "npt.NDArray[np.int64]",
) -> Tuple["npt.NDArray[np.int64]", "npt.NDArray[np.int64]"]:
    """Map hour 0..23 -> 1..24 and dow 0..6 -> 1..7; padding stays 0."""
    hours, dows = _extract_hour_and_dow(timestamps)
    hour_ids = np.zeros_like(hours)
    dow_ids = np.zeros_like(dows)
    valid_mask = timestamps > 0
    hour_ids[valid_mask] = hours[valid_mask] + 1
    dow_ids[valid_mask] = dows[valid_mask] + 1
    return hour_ids, dow_ids


def _build_request_weekend_dense(
    timestamps: "npt.NDArray[np.int64]",
) -> "npt.NDArray[np.float32]":
    feats = np.zeros((timestamps.shape[0], 1), dtype=np.float32)
    valid_mask = timestamps > 0
    if valid_mask.any():
        _, dows = _extract_hour_and_dow(timestamps)
        feats[valid_mask, 0] = (dows[valid_mask] >= 5).astype(np.float32)
    return feats


def _build_hour_weekend_bucket_ids(
    timestamps: "npt.NDArray[np.int64]",
) -> "npt.NDArray[np.int64]":
    hours, dows = _extract_hour_and_dow(timestamps)
    bucket_ids = np.zeros_like(hours)
    valid_mask = timestamps > 0
    weekend = (dows >= 5).astype(np.int64)
    bucket_ids[valid_mask] = weekend[valid_mask] * 24 + hours[valid_mask] + 1
    return bucket_ids
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        use_time_period_features: bool = False,
        time_period_mode: str = 'full',
        use_weekend_feature: bool = False,
        use_hour_x_weekend_bucket: bool = False,
    ) -> None:
        """
        Args:
            ...
            use_time_period_features: if True, append request-level periodic
                dense features and emit sequence-level hour/dow buckets.
            time_period_mode: one of off/request_only/seq_only/full.
        """
        super().__init__()
        ...
        self.use_time_period_features = use_time_period_features
        self.time_period_mode = time_period_mode
        if self.time_period_mode not in VALID_TIME_PERIOD_MODES:
            raise ValueError(
                f"Invalid time_period_mode={self.time_period_mode!r}; "
                f"expected one of {sorted(VALID_TIME_PERIOD_MODES)}"
            )
        if not self.use_time_period_features:
            self.time_period_mode = 'off'
        self.enable_request_time_period = self.time_period_mode in ('request_only', 'full')
        self.enable_seq_time_period = self.time_period_mode in ('seq_only', 'full')
        self.use_weekend_feature = use_weekend_feature
        self.use_hour_x_weekend_bucket = use_hour_x_weekend_bucket
        ...
        for domain in self.seq_domains:
            ...
            self._buf_seq_hour_weekend[domain] = np.zeros((B, max_len), dtype=np.int64)
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._request_time_period_offset: Optional[int] = None
        self._weekend_offset: Optional[int] = None
        if self.enable_request_time_period:
            self._request_time_period_offset = self.user_dense_schema.total_dim
            self.user_dense_schema.add(REQUEST_TIME_PERIOD_FID, REQUEST_TIME_PERIOD_DENSE_DIM)
        if self.use_weekend_feature and self.enable_request_time_period:
            self._weekend_offset = self.user_dense_schema.total_dim
            self.user_dense_schema.add(REQUEST_WEEKEND_FID, 1)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.enable_request_time_period and self._request_time_period_offset is not None:
            start = self._request_time_period_offset
            user_dense[:, start:start + REQUEST_TIME_PERIOD_DENSE_DIM] = _build_request_time_period_dense(timestamps)
        if self.use_weekend_feature and self._weekend_offset is not None:
            user_dense[:, self._weekend_offset:self._weekend_offset + 1] = _build_request_weekend_dense(timestamps)
        ...
        for domain in self.seq_domains:
            ...
            dow_bucket[:] = 0
            hour_weekend_bucket = self._buf_seq_hour_weekend[domain][:B]
            hour_weekend_bucket[:] = 0
            if ts_ci is not None:
                ...
                time_bucket[:] = buckets
                if self.enable_seq_time_period:
                    if self.use_hour_x_weekend_bucket:
                        hour_weekend_bucket[:] = _build_hour_weekend_bucket_ids(ts_padded)
                    else:
                        seq_hour_ids, seq_dow_ids = _build_period_bucket_ids(ts_padded)
                        hour_bucket[:] = seq_hour_ids
                        dow_bucket[:] = seq_dow_ids

            result[f'{domain}_time_bucket'] = torch.from_numpy(time_bucket.copy())
            if self.enable_seq_time_period:
                if self.use_hour_x_weekend_bucket:
                    result[f'{domain}_hour_weekend_bucket'] = torch.from_numpy(hour_weekend_bucket.copy())
                else:
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        use_weekend_feature=use_weekend_feature,
        use_hour_x_weekend_bucket=use_hour_x_weekend_bucket,
    )
