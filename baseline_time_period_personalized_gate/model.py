...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_personalized_time_gate: bool = False,
        ...
        self.time_gate_init_bias = time_gate_init_bias
        self.use_personalized_time_gate = use_personalized_time_gate
    ...
    def _init_params(self) -> None:
        ...
        if self.num_time_buckets > 0:
            ...
            nn.init.zeros_(self.time_gate.weight.data)
            nn.init.constant_(self.time_gate.bias.data, self.time_gate_init_bias)
        if self.use_time_period_features:
            ...
            nn.init.zeros_(self.hour_gate.weight.data)
            nn.init.constant_(self.hour_gate.bias.data, self.time_gate_init_bias)
            nn.init.zeros_(self.dow_gate.weight.data)
            nn.init.constant_(self.dow_gate.bias.data, self.time_gate_init_bias)
    ...
    def _embed_seq_domain(
        ...
        user_ctx: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        gate_input = base_token
        if self.use_personalized_time_gate and user_ctx is not None:
            gate_input = base_token + user_ctx.unsqueeze(1).expand(B, L, self.d_model)

        if self.time_fusion_mode == 'add':
            # Original direct-add path kept as the default behavior.
            if self.num_time_buckets > 0:
                token_emb = token_emb + self.time_embedding(time_bucket_ids)
            if self.use_time_period_features:
                if hour_bucket_ids is None:
                    hour_bucket_ids = time_bucket_ids.new_zeros(time_bucket_ids.shape)
                if dow_bucket_ids is None:
                    dow_bucket_ids = time_bucket_ids.new_zeros(time_bucket_ids.shape)
                token_emb = token_emb + self.hour_embedding(hour_bucket_ids)
                token_emb = token_emb + self.dow_embedding(dow_bucket_ids)
        elif self.time_fusion_mode == 'gated_add':
            if self.num_time_buckets > 0:
                time_emb = self.time_embedding(time_bucket_ids)
                token_emb = token_emb + torch.sigmoid(self.time_gate(gate_input)) * time_emb
            if self.use_time_period_features:
                if hour_bucket_ids is None:
                    hour_bucket_ids = time_bucket_ids.new_zeros(time_bucket_ids.shape)
                if dow_bucket_ids is None:
                    dow_bucket_ids = time_bucket_ids.new_zeros(time_bucket_ids.shape)
                hour_emb = self.hour_embedding(hour_bucket_ids)
                dow_emb = self.dow_embedding(dow_bucket_ids)
                token_emb = token_emb + torch.sigmoid(self.hour_gate(gate_input)) * hour_emb
                token_emb = token_emb + torch.sigmoid(self.dow_gate(gate_input)) * dow_emb
        else:
            raise ValueError(f"Unknown time_fusion_mode: {self.time_fusion_mode}")
