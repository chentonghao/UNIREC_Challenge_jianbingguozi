...
try:
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_continuous_seq.utils import sigmoid_focal_loss, EarlyStopping
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_continuous_seq.model import ModelInput
...
class PCVRHyFormerRankingTrainer:
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_period_dense: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            ...
            seq_dow_buckets[domain] = device_batch.get(
                ...
                torch.zeros(B, L, dtype=torch.long, device=self.device))
            if f'{domain}_period_dense' in device_batch:
        ...
        return ModelInput(
            ...
            seq_period_dense=seq_period_dense or None,
        )
