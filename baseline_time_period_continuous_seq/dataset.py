...
def _build_sequence_time_period_dense(
    timestamps: "npt.NDArray[np.int64]",
) -> "npt.NDArray[np.float32]":
    """Build per-event [hour_sin, hour_cos, dow_sin, dow_cos], shape [B, L, 4]."""
    feats = np.zeros(timestamps.shape + (REQUEST_TIME_PERIOD_DENSE_DIM,), dtype=np.float32)
    valid_mask = timestamps > 0
    if not valid_mask.any():
        return feats
    hours, dows = _extract_hour_and_dow(timestamps)
    hour_angles = 2.0 * np.pi * hours[valid_mask].astype(np.float32) / 24.0
    dow_angles = 2.0 * np.pi * dows[valid_mask].astype(np.float32) / 7.0
    feats[..., 0][valid_mask] = np.sin(hour_angles)
    feats[..., 1][valid_mask] = np.cos(hour_angles)
    feats[..., 2][valid_mask] = np.sin(dow_angles)
    feats[..., 3][valid_mask] = np.cos(dow_angles)
    return feats
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        seq_period_encoding: str = 'bucket',
    ) -> None:
        """
        Args:
            ...
            seq_period_encoding: bucket or continuous for sequence-level period features.
        """
        super().__init__()
        ...
        self.seq_period_encoding = seq_period_encoding
        if self.seq_period_encoding not in ('bucket', 'continuous'):
            raise ValueError(f"Invalid seq_period_encoding={self.seq_period_encoding!r}")
        ...
        for domain in self.seq_domains:
            ...
            self._buf_seq_period_dense[domain] = np.zeros(
                (B, max_len, REQUEST_TIME_PERIOD_DENSE_DIM), dtype=np.float32)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.enable_request_time_period:
            user_dense[:, -REQUEST_TIME_PERIOD_DENSE_DIM:] = _build_request_time_period_dense(timestamps)
        ...
        for domain in self.seq_domains:
            ...
            dow_bucket[:] = 0
            period_dense = self._buf_seq_period_dense[domain][:B]
            period_dense[:] = 0
            if ts_ci is not None:
                ...
                if self.enable_seq_time_period:
                    if self.seq_period_encoding == 'continuous':
                        period_dense[:] = _build_sequence_time_period_dense(ts_padded)
                    else:
            ...
            if self.enable_seq_time_period:
                if self.seq_period_encoding == 'continuous':
                    result[f'{domain}_period_dense'] = torch.from_numpy(period_dense.copy())
                else:
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        seq_period_encoding=seq_period_encoding,
    )
