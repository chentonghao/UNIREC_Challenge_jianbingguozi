"""PCVRHyFormer inference script for the baseline_time_period_continuous_seq variant.
...
try:
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_continuous_seq.dataset import (
        FeatureSchema,
        NUM_TIME_BUCKETS,
        PCVRParquetDataset,
    )
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_continuous_seq.model import (
        ModelInput,
        PCVRHyFormer,
    )
...
def _batch_to_model_input(batch: Dict[str, Any], device: str) -> ModelInput:
    ...
    seq_period_dense: Optional[Dict[str, torch.Tensor]] = None
    ...
    if has_dow_buckets:
        seq_dow_buckets = {}
    has_period_dense = any(f'{domain}_period_dense' in device_batch for domain in seq_domains)
    if has_period_dense:
        seq_period_dense = {}
    for domain in seq_domains:
        ...
        if seq_period_dense is not None and f'{domain}_period_dense' in device_batch:
            seq_period_dense[domain] = device_batch[f'{domain}_period_dense']
    ...
    return ModelInput(
        ...
        seq_period_dense=seq_period_dense,
    )
...
def main() -> None:
    ...
    seq_period_encoding = str(train_config.get('seq_period_encoding', 'bucket'))
