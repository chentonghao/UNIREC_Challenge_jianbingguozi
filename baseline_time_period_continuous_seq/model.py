...
class ModelInput(NamedTuple):
    ...
    seq_period_dense: Optional[dict] = None  # {domain: tensor [B, L, 4]}
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.use_time_period_features and self.seq_period_encoding == 'continuous':
            self.seq_period_proj = nn.Sequential(
                nn.Linear(4, d_model),
                nn.LayerNorm(d_model),
            )
    ...
    def _init_params(self) -> None:
        ...
        if self.use_time_period_features and self.seq_period_encoding == 'bucket':
    ...
    def _embed_seq_domain(
        ...
        period_dense: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        if (
            ...
            and self.seq_period_encoding == 'continuous'
            and period_dense is not None
        ):
            token_emb = token_emb + self.seq_period_proj(period_dense.float())
        ...
        if (
            self.use_time_period_features
            and self.seq_period_encoding == 'bucket'
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            ...
            period_dense = None
            if inputs.seq_period_dense is not None:
                period_dense = inputs.seq_period_dense.get(domain)
