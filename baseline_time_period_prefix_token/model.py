...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_time_prefix_token: bool = False,
        ...
        self.use_time_prefix_token = use_time_prefix_token
        self.enable_request_time_period = time_period_mode in ('request_only', 'full')
        ...
        # User dense feature projection (if available). When request time is
        # represented as a prefix token, the final 4 dims are removed here.
        self.user_dense_base_dim = user_dense_dim
        if self.use_time_prefix_token and self.use_time_period_features and self.enable_request_time_period:
            self.user_dense_base_dim = max(user_dense_dim - 4, 0)
        self.has_user_dense = self.user_dense_base_dim > 0
        if self.has_user_dense:
            self.user_dense_proj = nn.Sequential(
                nn.Linear(self.user_dense_base_dim, d_model),
                nn.LayerNorm(d_model),
            )
        self.has_time_prefix_token = (
            self.use_time_prefix_token
            and self.use_time_period_features
            and self.enable_request_time_period
            and user_dense_dim >= 4
        )
        if self.has_time_prefix_token:
            self.time_prefix_proj = nn.Sequential(
                nn.Linear(4, d_model),
                nn.LayerNorm(d_model),
            )
        ...
        self.num_ns = (num_user_ns + (1 if self.has_user_dense else 0)
                       + (1 if self.has_time_prefix_token else 0)
    ...
    def _split_user_dense_for_prefix(self, user_dense_feats: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        if self.has_time_prefix_token:
            return user_dense_feats[:, :self.user_dense_base_dim], user_dense_feats[:, -4:]
        return user_dense_feats, None
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        user_dense_base, req_time_dense = self._split_user_dense_for_prefix(inputs.user_dense_feats)

        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = F.silu(self.user_dense_proj(user_dense_base)).unsqueeze(1)  # (B, 1, D)
            ns_parts.append(user_dense_tok)
        if self.has_time_prefix_token and req_time_dense is not None:
            time_prefix_tok = F.silu(self.time_prefix_proj(req_time_dense)).unsqueeze(1)
            ns_parts.append(time_prefix_tok)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = F.silu(self.user_dense_proj(user_dense_base)).unsqueeze(1)
            ns_parts.append(user_dense_tok)
        if self.has_time_prefix_token and req_time_dense is not None:
            time_prefix_tok = F.silu(self.time_prefix_proj(req_time_dense)).unsqueeze(1)
            ns_parts.append(time_prefix_tok)
