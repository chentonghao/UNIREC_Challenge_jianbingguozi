...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        user_time_dense_dropout: float = 0.0,
        ...
        self.user_time_dense_dropout = nn.Dropout(user_time_dense_dropout)
    ...
    def _project_user_dense_token(self, user_dense_feats: torch.Tensor) -> torch.Tensor:
        if self.use_time_period_features and self.enable_request_time_period and user_dense_feats.shape[1] >= 4:
            base_dense = user_dense_feats[:, :-4]
            time_dense = self.user_time_dense_dropout(user_dense_feats[:, -4:])
            user_dense_feats = torch.cat([base_dense, time_dense], dim=-1)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = self._project_user_dense_token(inputs.user_dense_feats)  # (B, 1, D)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = self._project_user_dense_token(inputs.user_dense_feats)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        if self.has_item_dense:
