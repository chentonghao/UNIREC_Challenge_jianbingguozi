...
def _build_hour_dow_joint_bucket_ids(
    timestamps: "npt.NDArray[np.int64]",
) -> "npt.NDArray[np.int64]":
    """Map (dow, hour) to dow * 24 + hour + 1; padding stays 0."""
    hours, dows = _extract_hour_and_dow(timestamps)
    joint_ids = np.zeros_like(hours)
    valid_mask = timestamps > 0
    joint_ids[valid_mask] = dows[valid_mask] * 24 + hours[valid_mask] + 1
    return joint_ids
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        use_hour_dow_joint_bucket: bool = False,
    ) -> None:
        """
        Args:
            ...
            use_hour_dow_joint_bucket: emit a joint hour-by-dow bucket instead
                of separate sequence hour/dow buckets.
        """
        super().__init__()
        ...
        self.use_hour_dow_joint_bucket = use_hour_dow_joint_bucket
        ...
        for domain in self.seq_domains:
            ...
            self._buf_seq_hour_dow[domain] = np.zeros((B, max_len), dtype=np.int64)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            dow_bucket[:] = 0
            hour_dow_bucket = self._buf_seq_hour_dow[domain][:B]
            hour_dow_bucket[:] = 0
            if ts_ci is not None:
                ...
                if self.enable_seq_time_period:
                    if self.use_hour_dow_joint_bucket:
                        hour_dow_bucket[:] = _build_hour_dow_joint_bucket_ids(ts_padded)
                    else:
            ...
            if self.enable_seq_time_period:
                if self.use_hour_dow_joint_bucket:
                    result[f'{domain}_hour_dow_bucket'] = torch.from_numpy(hour_dow_bucket.copy())
                else:
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        use_hour_dow_joint_bucket=use_hour_dow_joint_bucket,
    )
