...
try:
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_joint_bucket.utils import sigmoid_focal_loss, EarlyStopping
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_joint_bucket.model import ModelInput
...
class PCVRHyFormerRankingTrainer:
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_hour_dow_buckets: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            ...
            seq_dow_buckets[domain] = device_batch.get(
                ...
                torch.zeros(B, L, dtype=torch.long, device=self.device))
            seq_hour_dow_buckets[domain] = device_batch.get(
                f'{domain}_hour_dow_bucket',
                torch.zeros(B, L, dtype=torch.long, device=self.device))
        return ModelInput(
