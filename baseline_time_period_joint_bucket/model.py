...
class ModelInput(NamedTuple):
    ...
    seq_hour_dow_buckets: Optional[dict] = None  # {domain: tensor [B, L]}
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.use_time_period_features and self.use_hour_dow_joint_bucket:
            self.hour_dow_embedding = nn.Embedding(169, d_model, padding_idx=0)
    ...
    def _init_params(self) -> None:
        ...
        if self.use_time_period_features and self.use_hour_dow_joint_bucket:
            nn.init.xavier_normal_(self.hour_dow_embedding.weight.data)
            self.hour_dow_embedding.weight.data[0, :] = 0
        elif self.use_time_period_features:
    ...
    def _embed_seq_domain(
        ...
        hour_dow_bucket_ids: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        if (
            ...
            and self.use_hour_dow_joint_bucket
            and hour_dow_bucket_ids is not None
        ):
            token_emb = token_emb + self.hour_dow_embedding(hour_dow_bucket_ids)
        ...
        if (
            self.use_time_period_features
            and not self.use_hour_dow_joint_bucket
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            ...
            hour_dow_bucket_ids = None
            if inputs.seq_hour_dow_buckets is not None:
                hour_dow_bucket_ids = inputs.seq_hour_dow_buckets.get(domain)
            tokens = self._embed_seq_domain(
                ...
                hour_dow_bucket_ids=hour_dow_bucket_ids)
