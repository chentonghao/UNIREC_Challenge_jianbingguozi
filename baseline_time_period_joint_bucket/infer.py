"""PCVRHyFormer inference script for the baseline_time_period_joint_bucket variant.
...
try:
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_joint_bucket.dataset import (
        FeatureSchema,
        NUM_TIME_BUCKETS,
        PCVRParquetDataset,
    )
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_joint_bucket.model import (
        ModelInput,
        PCVRHyFormer,
    )
...
_FALLBACK_MODEL_CFG = {
    ...
    'use_hour_dow_joint_bucket': False,
...
def _batch_to_model_input(batch: Dict[str, Any], device: str) -> ModelInput:
    ...
    seq_hour_dow_buckets: Optional[Dict[str, torch.Tensor]] = None
    ...
    if has_dow_buckets:
        seq_dow_buckets = {}
    has_hour_dow_buckets = any(f'{domain}_hour_dow_bucket' in device_batch for domain in seq_domains)
    if has_hour_dow_buckets:
        seq_hour_dow_buckets = {}
    for domain in seq_domains:
        ...
        if seq_hour_dow_buckets is not None and f'{domain}_hour_dow_bucket' in device_batch:
            seq_hour_dow_buckets[domain] = device_batch[f'{domain}_hour_dow_bucket']
    ...
    return ModelInput(
        ...
        seq_hour_dow_buckets=seq_hour_dow_buckets,
    )
...
def main() -> None:
    ...
    use_hour_dow_joint_bucket = bool(train_config.get('use_hour_dow_joint_bucket', False))
