...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        history_time_dropout: float = 0.0,
        ...
        self.history_time_dropout = nn.Dropout(history_time_dropout)
    ...
    def _embed_seq_domain(
        ...
        if self.num_time_buckets > 0:
            time_emb = self.history_time_dropout(self.time_embedding(time_bucket_ids))
            token_emb = token_emb + time_emb
        if self.use_time_period_features and hour_bucket_ids is not None:
            hour_emb = self.history_time_dropout(self.hour_embedding(hour_bucket_ids))
            token_emb = token_emb + hour_emb
        if self.use_time_period_features and dow_bucket_ids is not None:
            dow_emb = self.history_time_dropout(self.dow_embedding(dow_bucket_ids))
