...
class RoPEMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if self.pos_bias_type == 'rope' and rope_cos is not None and rope_sin is not None:
        ...
        # 4. Convert masks and optional ALiBi into an additive SDPA bias.
        sdpa_attn_mask = None
        ...
        if attn_mask is not None:
            additive = attn_mask.to(device=query.device, dtype=query.dtype)
            additive = additive.unsqueeze(0).unsqueeze(0).expand(B, self.num_heads, Lq, Lk)
            if sdpa_attn_mask is not None:
                sdpa_attn_mask = sdpa_attn_mask + additive
            else:
                sdpa_attn_mask = additive

        if self.pos_bias_type == 'alibi':
            q_pos = torch.arange(Lq, device=query.device).unsqueeze(1)
            k_pos = torch.arange(Lk, device=query.device).unsqueeze(0)
            dist = (q_pos - k_pos).abs().to(query.dtype)
            slopes = 2.0 ** (
                -torch.arange(1, self.num_heads + 1, device=query.device, dtype=query.dtype)
                / self.num_heads
            )
            alibi = -slopes.view(1, self.num_heads, 1, 1) * dist.view(1, 1, Lq, Lk)
            sdpa_attn_mask = alibi if sdpa_attn_mask is None else sdpa_attn_mask + alibi
...
def create_sequence_encoder(
    ...
    if encoder_type == 'swiglu':
        ...
        return TransformerEncoder(d_model, num_heads, hidden_mult, dropout, pos_bias_type=pos_bias_type)
    elif encoder_type == 'longer':
        return LongerEncoder(d_model, num_heads, top_k, hidden_mult, dropout, causal, pos_bias_type=pos_bias_type)
    else:
