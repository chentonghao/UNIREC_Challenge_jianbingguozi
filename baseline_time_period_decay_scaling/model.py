...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_time_decay_scaling: bool = False,
        time_decay_init: float = 0.0,
        ...
        self.use_time_decay_scaling = use_time_decay_scaling and num_time_buckets > 0
        self.time_decay_init = time_decay_init
        ...
        if self.use_time_decay_scaling:
            self.time_decay_scale = nn.Embedding(num_time_buckets, d_model, padding_idx=0)
    ...
    def _init_params(self) -> None:
        ...
        if self.use_time_decay_scaling:
            nn.init.zeros_(self.time_decay_scale.weight.data)
            if self.time_decay_scale.weight.shape[0] > 1:
                self.time_decay_scale.weight.data[1:, :].add_(self.time_decay_init)
            self.time_decay_scale.weight.data[0, :] = 0
        if self.use_time_period_features:
    ...
    def _embed_seq_domain(
        ...
        if self.use_time_decay_scaling:
            decay = torch.sigmoid(self.time_decay_scale(time_bucket_ids))
            token_emb = token_emb * (1.0 + decay)
