...
try:
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_decay_scaling.utils import set_seed, EarlyStopping, create_logger
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_decay_scaling.dataset import FeatureSchema, get_pcvr_data, NUM_TIME_BUCKETS
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_decay_scaling.model import PCVRHyFormer
    from UNIREC_Challenge_jianbingguozi.baseline_time_period_decay_scaling.trainer import PCVRHyFormerRankingTrainer
except ModuleNotFoundError:
    # Competition platforms may flatten these files into one runtime directory
    # instead of preserving the original package layout.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_time_period_features', action='store_true', default=False,
                        help='Enable request-level hour/dow sin-cos dense features and '
                             'sequence-level hour/dow bucket embeddings')
    parser.add_argument('--time_period_mode', type=str, default='full',
                        choices=['off', 'request_only', 'seq_only', 'full'],
                        help='Periodic time-feature mode. '
                             'When explicitly set, overrides the legacy '
                             '--use_time_period_features compatibility switch.')
    parser.add_argument('--use_time_decay_scaling', action='store_true', default=False,
                        help='Apply learnable multiplicative scaling based on seq time buckets')
    parser.add_argument('--time_decay_init', type=float, default=0.0,
                        help='Initial value added to non-padding rows of the time decay embedding')
    ...
    # - legacy default remains "off" unless --use_time_period_features is set
    # - explicit --time_period_mode wins, even without the legacy flag
    explicit_time_period_mode = '--time_period_mode' in sys.argv
    if explicit_time_period_mode:
        effective_time_period_mode = args.time_period_mode
    else:
        effective_time_period_mode = 'full' if args.use_time_period_features else 'off'

    args.time_period_mode = effective_time_period_mode
    args.use_time_period_features = effective_time_period_mode != 'off'
...
def main() -> None:
    ...
    missing_required = [name for name in ('data_dir', 'ckpt_dir', 'log_dir') if getattr(args, name) is None]
    if missing_required:
        raise ValueError(f"Missing required args/env for: {', '.join(missing_required)}")
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        use_time_period_features=args.use_time_period_features,
        time_period_mode=args.time_period_mode,
    )
    ...
    model_args = {
        ...
        "use_time_period_features": args.use_time_period_features,
        "use_time_decay_scaling": args.use_time_decay_scaling,
        "time_decay_init": args.time_decay_init,
