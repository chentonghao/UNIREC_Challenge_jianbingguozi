...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        self.time_feat_dropout = nn.Dropout(time_feat_dropout)
    ...
    def _embed_seq_domain(
        ...
        if self.time_fusion_mode == 'add':
            ...
            if self.num_time_buckets > 0:
                time_emb = self.time_feat_dropout(self.time_embedding(time_bucket_ids))
            ...
            if self.use_time_period_features:
                ...
                hour_emb = self.time_feat_dropout(self.hour_embedding(hour_bucket_ids))
                dow_emb = self.time_feat_dropout(self.dow_embedding(dow_bucket_ids))
                token_emb = token_emb + hour_emb
                token_emb = token_emb + dow_emb
        elif self.time_fusion_mode == 'gated_add':
            if self.num_time_buckets > 0:
                time_emb = self.time_feat_dropout(self.time_embedding(time_bucket_ids))
                token_emb = token_emb + torch.sigmoid(self.time_gate(base_token)) * time_emb
            if self.use_time_period_features:
                if hour_bucket_ids is None:
                    hour_bucket_ids = time_bucket_ids.new_zeros(time_bucket_ids.shape)
                if dow_bucket_ids is None:
                ...
                hour_emb = self.time_feat_dropout(self.hour_embedding(hour_bucket_ids))
                dow_emb = self.time_feat_dropout(self.dow_embedding(dow_bucket_ids))
                token_emb = token_emb + torch.sigmoid(self.hour_gate(base_token)) * hour_emb
                token_emb = token_emb + torch.sigmoid(self.dow_gate(base_token)) * dow_emb
        else:
